...
__write_ifcfg() {
    ...
    _file=/tmp/write_ifcfg.sh

    _parent_mac=$(nsenter --mount=/host/1/ns/mnt cat /sys/class/net/"$_nic"/address)

    echo "cat >/etc/sysconfig/network-scripts/ifcfg-$_nic.$_vlan <<-'AEOF'" >$_file
    echo 'DEVICE='"$_nic.$_vlan" >>$_file
    echo 'ONBOOT=yes' >>$_file
    echo 'VLAN=yes' >>$_file
    echo 'MACADDR='$(echo $_parent_mac-"$_nic.$_vlan" | md5sum | sed -e 's/^\(..\)\(..\)\(..\)\(..\)\(..\).*$/66:\1:\2:\3:\4:\5/') >>$_file
    echo 'AEOF' >>$_file
    echo 'ifup '"$_nic.$_vlan" >>$_file
    # echo 'rm -rf /etc/sysconfig/network-scripts/ifcfg-'$_nic >>$_file

    nsenter --mount=/host/1/ns/mnt bash $_rootfs$_file
}
