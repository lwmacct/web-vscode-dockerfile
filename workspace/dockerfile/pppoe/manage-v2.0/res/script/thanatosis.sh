...
__thanatosis() {
    ...
    _tx_now=$(echo "$_ifconfig_ppp_all" | grep "$_ppp" -A6 | grep 'TX packets')
    _rx_now=$(echo "$_ifconfig_ppp_all" | grep "$_ppp" -A6 | grep 'RX packets')
...
_for() {
    _ifconfig_ppp_all=$(ifconfig | grep -E 'ppp[0-9]{1,3}' -A6)
