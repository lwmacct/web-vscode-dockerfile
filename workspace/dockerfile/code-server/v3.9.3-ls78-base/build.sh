...
FROM linuxserver/code-server:v3.9.3-ls78
USER root
...
__add_file() {
    ...
    cp -rf /data/docker-data/vscode/workspace/dockerfile/code-server/v3.9.3-ls78-base/res $_workspace/res/
    chmod -R 777 $_workspace/res/
}

__build() {
    # 下面的参数  -t 参数,改成你自己的阿里云镜像服务仓库地址
    docker build -t registry.cn-hangzhou.aliyuncs.com/lwmacct/code-server:v3.9.3-ls78-base $_workspace
    docker push registry.cn-hangzhou.aliyuncs.com/lwmacct/code-server:v3.9.3-ls78-base
}
...
__exec() {
    ...
    # 执行构建命令
    bash /data/docker-data/vscode/workspace/dockerfile/code-server/v3.9.3-ls78-base/build.sh
}
